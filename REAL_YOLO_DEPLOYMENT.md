...
roslaunch nuedc_ground_air landscreen_ros1_bridge.launch \
  camera:=/dev/video0 show_window:=true
```

The idle launch starts path planning, path reception, the TCP bridge, and the START/STOP listener. It does not open the camera or run YOLO.
...
When LandScreen sends `launch=false`, or the mission state becomes `FINISHED`, `COMPLETE`, `COMPLETED`, `LANDED`, or `ABORT`, all four recognition nodes and the OpenCV window stop.
